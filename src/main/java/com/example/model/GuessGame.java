...
public class GuessGame {
  ...
  public synchronized GuessResult makeGuess(final int myGuess) {
    ...
    if (myGuess < 0 || myGuess >= UPPER_BOUND) {
      ...
      if (isCorrect) {
        ...
        GameCenter.increaseTotalGamesWon();
  ...
  public synchronized String hint(int guess){
    String string = "";
    if (guess > numberToGuess){
      string = "The number is lower than your guess";
    }else {
      if (guess < numberToGuess) {
        string = "The number is higher than your guess";
      }
    }
    return string;
  }
